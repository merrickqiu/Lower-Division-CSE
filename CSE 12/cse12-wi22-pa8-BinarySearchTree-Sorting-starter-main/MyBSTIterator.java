 ...
 * Sources used: None
 * 
 * An iterator for MyBST.
 * It repeatedly gives the successor of the current node.
 */
...
/**
 * An iterator for MyBST
 * It contains an abstract inner class MyBSTNodeIterator
 */
public class MyBSTIterator<K extends Comparable<K>, V> extends MyBST<K, V> {
    abstract class MyBSTNodeIterator<T> implements Iterator<T> {
        ...
        /**
         * Gives the successor of the current node
         ...
         */
        public void remove() {
            if (lastVisited == null) { // next hasn't been called
                throw new IllegalStateException(); //throw exception
            }
            if (lastVisited.getRight() != null && // no right child
                    lastVisited.getLeft() != null) { // or left, so leaf node
                next = lastVisited; // set next to last visited
                // this is so that leaf nodes can still get returned
            }
            MyBSTIterator.this.remove(lastVisited.getKey()); //bst node remove
            lastVisited = null; // lastVisited is removed so make it null
        }
    }
