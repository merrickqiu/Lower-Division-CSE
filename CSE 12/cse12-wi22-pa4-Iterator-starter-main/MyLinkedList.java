 ...
 * This version also has an iterator.
 */
...
public class MyLinkedList<E> extends AbstractList<E> {
	...
	protected class MyListIterator implements ListIterator<E> {
		 ...
		 * @return if next() can be called
		 */
		public boolean hasNext() {
			return right != tail && right != null;
		}
		 ...
		 * @return if previous() can be called
		 */
		...
		/**
		 * The index of the element that would be returned by prev()
		 * @return the index of previous()
		 */
		...
		/**
		 * Adds the element before the element that would be returned by next
		...
		/**
		 * Sets the node from the most recent next or previous call to element
		...
		/**
		 * Removes the most recent next or previous call
		 */
		public void remove() {
			if (!canRemoveOrSet) {
				throw new IllegalStateException();
			...
			if (forward) {
				// Have previous node skip over this node
				left.getPrev().setNext(left.getNext());
				// Have next node skip over this node
				left.getNext().setPrev(left.getPrev());
				left = left.getPrev();
				idx--; // only being forward will change idx
			}
			else {
