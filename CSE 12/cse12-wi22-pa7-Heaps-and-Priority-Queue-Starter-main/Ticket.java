 ...
 * Sources used: None
 * 
 * A class that represents a student ticket.
 * It is used in a priority queue to simulate an autograder queue. 
 */
...
public class Ticket implements Comparable<Ticket>{
    ...
    /**
     * Compares two tickets based on their ticket type and then creation time
     * @return Negative if <, 0 if ==, and positive if >
     */
    @Override
    public int compareTo(Ticket other){
        if (orderMap.get(ticketType) < orderMap.get(other.ticketType)) {
            return -1;
        } else if (orderMap.get(ticketType).compareTo(orderMap.get(other.ticketType)) == 0) {
            if (createdAt < other.createdAt) {
                return -1;
            } else if (createdAt.compareTo(other.createdAt) == 0) {
                return 0;
            } else {
                return 1;
            }
